...
``aiida-dftbplus`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
Once you're happy with your documentation, it's easy to host it online on ReadTheDocs_:
 ...
 #. Import your ``aiida-dftbplus`` repository (preferably using ``aiida-dftbplus`` as the project name)

The documentation is now available at `aiida-dftbplus.readthedocs.io <http://aiida-dftbplus.readthedocs.io/>`_.
